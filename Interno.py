			...
			while True:
				...
				if event == 'confirmar':
					...
					layout_protocolo = [[sg.Text('Chave NFE:'), sg.InputText(data[8], key='chave_nfe', disabled=True),
					                    ...
					                    [sg.Button('Pesquisar Protocolo / Baixar XML', key='pesquisar_protocolo')],
					                    [sg.Text('Inserir Protocolo:'), sg.InputText(key='protocolo'),
					                     sg.Button('Inserir', key='inserir_protocolo')],
					                    ...
					                    [sg.Text('Inserir arquivo XML:'),
					                     ...
					                     sg.FileBrowse('Selecionar arquivo', file_types=(("XML Files", "*.xml"),),
					                                   target='input_file', key='file_browse', disabled=True)],
					                    [sg.Text('', key='file_name')],
					                    [sg.Button('Inserir arquivo', key='file_insert', disabled=True)],
					                    ]
					...
					while True:
						...
						if event == 'pesquisar_protocolo':
							import webbrowser

							webbrowser.open('https://www.nfe.fazenda.gov.br/portal/consultaRecaptcha.aspx')

						if event == 'inserir_protocolo':
							protocolo = values['protocolo']
							if len(protocolo) != 15 or not protocolo.isnumeric():
								sg.popup_error('O protocolo deve ter 15 caracteres numéricos!')
								continue
							try:
								cursor.execute(
									...
									(values['protocolo'], data[8], data[4], data[2])
								)
								conn.commit()

								window_protocolo['resultado'].update(
									'Protocolo inserido com sucesso! Para gerar a Danfe, selecione o XML da nota abaixo.')
								window_protocolo['file_browse'].update(disabled=False)
								window_protocolo['file_insert'].update(disabled=False)
								window_protocolo['protocolo'].update(disabled=True)
								window_protocolo['inserir_protocolo'].update(disabled=True)
							except Exception as e:
								window_protocolo['resultado'].update(f'Erro ao inserir protocolo: {str(e)}')
						...
						if event == 'file_insert':
							...
							with open(values['input_file'], 'r') as file:
								...
								if data[5] == '2':
									...
									sg.popup('Arquivo inserido com sucesso! Gere a Danfe pelo Gestor.')
								elif data[5] == '1':
